...
func interact(conn *net.TCPConn) {
	...
	for {
		...
		case "quit":
			...
			return
		case "image_list":
			send_request(encoder, "get_image_list", nil)
		case "get":
			var pointRequest proto.ImagePoint
			fmt.Println("image name: ")
			pointRequest.ImageName = input.Gets()
			pointRequest.X, _ = strconv.Atoi(input.Gets())
			pointRequest.Y, _ = strconv.Atoi(input.Gets())
			fmt.Println()
			send_request(encoder, "get_color", pointRequest)
		default:
		...
		case "list found":
			...
			var names []string
			json.Unmarshal(*resp.Data, &names)
		...
		case "pixel found":
			...
			var colorRespond string
			json.Unmarshal(*resp.Data, &colorRespond)

			fmt.Println(colorRespond)
